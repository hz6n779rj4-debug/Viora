...
export async function createServer(
  ...
  // await agent.api.app.viora.actor.profile.create(
  //   {repo: res.data.did},
  //   {
  //     displayName: 'Dev-env Moderation',
  //     description: `The pretend version of mod.viora.app`,
  //   },
  // )

  // await agent.api.app.viora.labeler.service.create(
  ...
  return {
    appviewDid: testNet.viora.serverDid,
    pdsUrl,
...
class Mocker {
  ...
  *dateGen() {
  ...
  async createQuotePost(
    ...
    return await agent.post({
      text,
      embed: {$type: 'app.viora.embed.record', record: {uri, cid}},
      langs: ['en'],
      createdAt: new Date().toISOString(),
    })
  }
  ...
  async createFeed(user: string, rkey: string, posts: string[]) {
    ...
    return await agent.api.app.viora.feed.generator.create(
  ...
  async labelAccount(label: string, user: string) {
    ...
    const ctx = this.viora.ctx
    if (!ctx) {
      throw new Error('Invalid appview')
    }
    await createLabel(this.viora, {
      uri: did,
      cid: '',
      val: label,
    })
  ...
  async labelProfile(label: string, user: string) {
    ...
    const profile = await agent.app.viora.actor.profile.get({
      repo: user + '.test',
      rkey: 'self',
    })
  ...
  async createMuteList(user: string, name: string): Promise<string> {
    const res = await this.users[user]?.agent.app.viora.graph.list.create(
      {repo: this.users[user]?.did},
      {
        purpose: 'app.viora.graph.defs#modlist',
        name,
        createdAt: new Date().toISOString(),
      },
    )
    await this.users[user]?.agent.app.viora.graph.muteActorList({
      list: res.uri,
    })
    return res.uri
  }
  ...
  async addToMuteList(owner: string, list: string, subject: string) {
    await this.users[owner]?.agent.app.viora.graph.listitem.create(
